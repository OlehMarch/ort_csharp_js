...
{
    public class TestObjAlist
    {
        AList2O lst = new AList2O();
        ...
        [TestCase(null)]
        public void DelIniEx(int[] array)
        {
            Assert.That(() => lst.Init(array), Throws.Exception);
        }
        [TestCase(new int[] { }, 0)]
        ...
        public void TestDelEnd(int[] ar, int[] exp, object expVal, int expSize)
        {
            lst.Init(ar);
            object actVal = lst.DelEnd();
            object[] act = lst.ToArray();
            int actSize = lst.Size();
            Assert.AreEqual(expSize, actSize);
            Assert.AreEqual(expVal, actVal);
        ...
        public void TestGet(int[] ar, int pos, object exp)
        {
            lst.Init(ar);
            object act = lst.Get(pos);
        ...
        public void Complex2(int[] array, int[] res, object result, int size)
        {
            lst.Init(array);
            lst.HalfReverse();
            object act = lst.DelEnd();
            lst.DelPos(5);
            int expSize = lst.Size();
            Assert.AreEqual(expSize, size);
            Assert.AreEqual(result, act);
        ...
        [TestCase()]
        public void Test_InvalidOp()
        {
            lst.AddStart(5);
            Assert.That(() => lst.AddPos(5, "z"), Throws.TypeOf<InvalidOperationException>());
        }
        [TestCase()]
        public void Test_InvalidOp2()
        {
            lst.AddStart(5);
            Assert.That(() => lst.AddEnd("z"), Throws.TypeOf<InvalidOperationException>());
        }
        [TestCase()]
        public void Test_InvalidOp3()
        {
            lst.AddStart(5);
            Assert.That(() => lst.AddStart("z"), Throws.TypeOf<InvalidOperationException>());
        }
    }
}
