function MarkupParser(xmlText)
{
    ...
    this.SAX_Parser_ElementContext = function(e)
    {
		...
		if (this.elementStack.IsFullCheck())
		{
			this.currentControl = Fabrica.createElement(this.elementStack);
			this.ownerList[this.depthLevel - 1].innerHTML += this.currentControl;
			this.ownerList[this.depthLevel] = document.getElementById(this.elementStack.id);
...
Fabrica.createElement = function(e)
{
	...
	switch (type) {
		...
		case "Panel":
			...
			break;
		case "Button":
			element = "<input align='left' type='button' id='" + id + "'value='" + value + "'width='" + width + "'height='" + height
				+ "' style='position: absolute; margin-left: " + x + "px; margin-top: " + y + "px;' >";
			break;
		case "TextBox":
			element = "<input align='left' type='text' id='" + id + "'value='" + value + "'width='" + width + "'height='" + height
				+ "' style='position: absolute; margin-left: " + x + "px; margin-top: " + y + "px;' >";
			break;
	}
